...
export default function NotePanel({ storyId, pageIndex, onClose }: Props) {
  const storageKey = `notes_story_${storyId}`;
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 flex items-center justify-center bg-amber-100 rounded-lg">
              <i className="ri-sticky-note-line text-amber-600 text-base"></i>
            </div>
            <div>
        ...
        <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-4">
          <div>
            ...
            <textarea
              ...
              maxLength={500}
              className="w-full h-36 px-4 py-3 rounded-xl border border-gray-200 text-sm text-gray-700 resize-none focus:outline-none focus:ring-2 focus:ring-amber-300 focus:border-transparent"
              style={{ background: '#fffdf5', fontFamily: 'inherit', lineHeight: '1.7' }}
            />
          ...
          <div className="flex gap-2">
            <button
              onClick={handleSave}
              className="flex-1 py-2.5 bg-amber-500 hover:bg-amber-600 text-white text-sm font-semibold rounded-xl transition-colors cursor-pointer whitespace-nowrap flex items-center justify-center gap-2"
            >
            ...
            <div>
              ...
              <div className="space-y-2">
                {allNotes.map(([pg, note]) => (
                  <div key={pg} className={`p-3 rounded-xl border text-xs cursor-pointer transition-all ${Number(pg) === pageIndex ? 'border-amber-300 bg-amber-50' : 'border-gray-100 bg-gray-50 hover:bg-amber-50'}`}>
                    <div className="flex items-center justify-between mb-1">
                      <span className="font-semibold text-amber-600">Trang {Number(pg) + 1}</span>
                      {Number(pg) === pageIndex && <span className="text-amber-400 text-xs">Đang xem</span>}
                    </div>
